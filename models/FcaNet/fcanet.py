...
from models.FcaNet.layer import MultiSpectralAttentionLayer
...
class FcaBasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        # assert fea_h is not None
        # assert fea_w is not None
        c2wh = dict([(64,56), (128,28), (256,14) ,(512,7)])  # ()里面的两个数，分别是通道数和尺寸
        self.planes = planes
...
def fcanet18(num_classes=1_000, pretrained=False):
...
if __name__ == '__main__':
    import torch
    net = fcanet18(pretrained=False, num_classes=2)
    net.conv1 = nn.Conv2d(1, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
    temp = torch.randn((8, 1, 224, 224))
    y = net(temp)
    print(y.shape)
    # print(net)
